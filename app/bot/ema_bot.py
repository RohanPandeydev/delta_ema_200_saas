#!/usr/bin/env python3
"""
EMA-200 Strategy Trading Bot for Delta Exchange - DOCKER VERSION
...
class EMAStrategyBot:
    ...
    def _log(self, message, color=Fore.WHITE):
        """Log message to console and notify"""
    ...
    def monitor_and_replace_order(self, order_id, side, size, max_wait_seconds=15, check_interval=15):
        """
        Monitor an order and replace it if unfilled after max_wait_seconds
        ...
        while True:
            ...
            if order:
                ...
                # Order still open - check if we should replace it
                if elapsed >= max_wait_seconds:
                    ...
                    if self.cancel_order(order_id):
                        ...
                        if result.get('success'):
                            # Recursively monitor the new order
    ...
    def close_position(self, current_pos):
        ...
        if result.get('success'):
            ...
            # Monitor the order and replace if needed
